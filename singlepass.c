...
struct input
 {
char label[20],opcode[20],operand[20],mnemonic[25]; int loc;
 };
...
break;
 ...
 if(f1==0)
{
strcpy(symtbl[j].sym,table[i].label); symtbl[j].val=locctr; 
symtbl[j].f=0;
j++;
}
 }
...
fprintf(fp3,"%s\t%s\t%s\t%s\n",table[x].label,table[x].opcode,table[x].operand,strcat(table[x
].mnemonic,add));
}
printf("\n\n symol table\n\n");
for(x=1;x<j;x++)
